...
**到達目標：** 1次元データ（身長など）を、平均・分散・標準偏差で要約できる（Python/R 両方）。
...
分散・標準偏差には「2種類」あり、どちらを使うかで値が変わります。講義メモでは、混乱回避のために両者の対応表を示しています。
...
26. `python ファイル名.py` で実行できている
27. NumPy を使って平均を計算できる
28. 分散・標準偏差で ddof=1 を明示できる
29. `np.var(ddof=0/1)` の違いを出力で確認した
30. pandas で DataFrame を作れる
31. `df[["english","math"]]` のように列選択できる
32. `df.var(ddof=1)` / `df.std(ddof=1)` を使える
33. `df.describe()` を実行できる
34. `value_counts()` でカテゴリ件数を出せる
35. `pd.crosstab()` で分割表を作れる
36. `groupby(...).mean()` でグループ平均を出せる
37. `groupby` の結果で `index` になることを理解している
38. `pandas` と NumPy のデフォルトの違いに注意できる

### D. R（14）

39. `Rscript ファイル名.R` で実行できている
40. Rscript で出力するために `cat()`/`print()` を使っている
41. `mean()` を使える
42. `var()` を使える
43. `sd()` を使える
44. 標本分散を `mean((x-mean(x))^2)` で計算できる
45. `data.frame()` を作れる
46. `df$col` で列を取り出せる
47. `sapply(df[c(...)], var)` で複数列を集計できる
48. `summary(df)` を読める
49. `table()` でカテゴリ件数を出せる
50. `table(gender, excel)` の分割表を作れる
51. `aggregate(... ~ gender, mean)` でグループ平均を出せる
52. `TRUE/FALSE` の意味を理解している

### E. デバッグ・Copilot（8）

53. 出力が合わないときに `ddof` を疑える
54. データ入力ミスを先に疑う手順でデバッグできた
55. Python と R の結果を突き合わせて検証できた
56. Copilot を使うとき「行数」「ddof」「出力形式」を指定した
57. Copilot の生成物を丸投げせず、コメントを自分で書き直した
58. Copilot に「短くして」と再指示できた
59. 統合演習の3種類の出力（全体/分割表/グループ平均）を満たした
60. ファイル名が指示どおりになっている

**チェック数による達成度評価（目安）**

*  0〜20：要復習（まずは実行と出力の型を固める）
* 21〜40：基礎OK（ddof と DataFrame 集計を反復）
* 41〜50：実務入口（Python/R の比較検証ができる）
* 51〜60：優秀（統合演習を自力で再現し、説明できる）